...
tab1,tab2=st.tabs(['4 velden, geen Y-draden','4 velden, met Y-draden'])

with tab1:
    

    st.title('Hangdraadindeling OSI 4 velden')
    st.write('''dit is een tool om de hangdraadverdeling te berekenen voor een OSI over 4 velden zonder Y-draden.  
    ...
    for i in range(len(indeling2[1])):
            print(f'    {i+1}                {indeling2[1][i]:5.2f}               {onderling2[1][i]:5.2f}')
    print('\n Veld 3:')
    for i in range(len(indeling3[1])):
            print(f'    {i+1}                {indeling3[1][i]:5.2f}               {onderling3[1][i]:5.2f}')
